...
var mm = today.getMonth()+1;  //Jan is Month 0
...
/* add cookies even though it won't really work*/
...
function validateDeliveryDate() {
   ...
   var currentElement;
  // var deliveryDate = new Date (selectElements);
   try {
      for (var i = 0; i < elementCount; i++) {
...
/* validate shirt fieldset*/
function validateSize()
{
	var errorDiv = document.querySelector("#size .errorMessage");
	var fieldsetValidity = true;
	var size = document.getElementsByName("shirtSize");
	...
	try 
	{
	  ...
	  else 
	  {
         for (var i = 0; i < 4; i++) 
		 {
            size[i].style.outline = "";
         }
      }
	  if (!fieldsetValidity) { // check if any field is blank
         throw "Please select a shirt size.";
      } else {
         errorDiv.style.display = "none";
      }
   }
...
function validatePayment() {
   ...
   try {
	  ...
	  // verify that card number is 16 digits
	  else if (ccNumElement.value > 9999999999999999 || ccNumElement.value < 1000000000000000 )
	  {
		  ccNumElement.style.background = "rgb(255,233,233)";
         fieldsetValidity = false;
	  }
      ...
      if (cvvElement.value === "") {
	   ...
	   //verify that a cvv value is not larger three digits
	  if (cvvElement.value > 999)
	  {
		 cvvElement.style.background = "rgb(255,233,233)";
         fieldsetValidity = false;
	  }else {
...
function validateName() {
   var errorDiv = document.querySelector("#contact .errorMessage");
   var user_name = document.getElementById("user_name");
   var email_address = document.getElementById("email_address");
   var fullContactDiv = document.querySelector("#contact .fullContact");
   var fullContact = "CONTACT SUMMARY -- ";
   var str = user_name.value;
   var spacePos = str.indexOf (" ");  //Homework String Function IndexOf
   var firstName = "";
   var lastName = "";
   ...
   try {
      if (((user_name.value === "") || (user_name.value === user_name.placeholder))||((email_address.value === "") || (email_address.value === email_address.placeholder)) ) {
          // custom checked but message box empty
         throw "Please enter your contact information.";
      } else {
		 //Homework String Function - Slice
		 firstName = str.slice(0,4);
		 lastName = str.slice((spacePos + 1),str.length);
		 errorDiv.style.display = "none";
		 fullContactDiv.style.display = "block";
		 //Homework String Function - Concat
		 fullContact = fullContact.concat(" Full Name: ",user_name.value," Email Address: ",email_address.value," First: ",firstName," Last: ",lastName);
		 fullContactDiv.innerHTML = fullContact;		 
      }
   }
   catch (msg) {
      fullContactDiv.style.display = "none";
	  errorDiv.style.display = "block";
      errorDiv.innerHTML = msg;
      //user.style.background = "rgb(255,233,233)";
      formValidity = false;
   }
}
